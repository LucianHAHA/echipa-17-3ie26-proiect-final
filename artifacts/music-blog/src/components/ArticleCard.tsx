...
export function ArticleCard({ article }: ArticleCardProps) {
  const { title, slug, description, publishedAt, cover, category, author } = article.attributes;
  ...
  return (
    <Link href={`/articles/${slug || article.id}`}>
      <Card className="group h-full flex flex-col overflow-hidden bg-card border-card-border hover:-translate-y-1 transition-all duration-300 card-glow cursor-pointer" data-testid={`article-card-${article.id}`}>
        <div className="relative w-full aspect-[16/9] overflow-hidden bg-muted">
          ...
          {categoryName && (
            <Badge 
              className="absolute top-4 right-4 z-20 bg-primary hover:bg-primary/90 text-primary-foreground font-medium"
            >
              {categoryName}
            </Badge>
          )}
